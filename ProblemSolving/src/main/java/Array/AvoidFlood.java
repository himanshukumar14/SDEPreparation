 ...
 * flood-free. [-1,-1,1,2,-1,-1] is another acceptable scenario. Example 3:
 * 
...
public class AvoidFlood {
	...
	public int[] avoidFlood(int[] rains) {
		...
		TreeSet<Integer> dry = new TreeSet<>(); //sorted set
		int[] ans = new int[n];
		...
		for (int i = 0; i < n; ++i) {
			if (rains[i] == 0) {
				...
				if (map.containsKey(rains[i])) {
					// FIND SUCH DRY DAY WHOSE MAP.GET(RAINS[I]) < DRY.GET(j), basically dry day is later than the rain day
					Integer val = dry.ceiling(map.get(rains[i])); // logn time since can be done using binary search
